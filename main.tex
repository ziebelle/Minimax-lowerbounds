...
\end{proof}
Now it is important to note that usually it does not matter which $f$-divergence we take here. To see this, we require a slight generalisation of \Cref{result:reduction_scheme}. (The original result is recovered by setting $\tilde{\Theta}=\{\vartheta_1, \dots, \vartheta_M\}$ and $\pi$ as the discrete uniform. Note also that many ideas from the proof of \Cref{result:reduction_scheme} and \Cref{result:tv_lowerbound} are also visible in this result.) 
\begin{propositionrep}
Let $(\mathcal{X}_n, \mathcal{F}_n, (\mathbb{P}_{\vartheta,n})_{\vartheta \in \Theta})$ be a statistical model and $(\Theta,d)$ a metric space. Suppose $\tilde{\Theta}\subset \Theta$ and $((\tilde{\Theta},d), \Pi, \pi)$ is a polish space.\todo{Potentially make a statistical model assumption at the beginning somewhere.} Then, the minimax risk is lower bounded by the Bayes risk according to $\pi$:
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}\todo{Check if later formulation is also true if $\pi$ is non-discrete.}
\end{propositionrep}
\todo{Add a proof.}
\begin{definition}
...
\end{definition}
The following result provides a lowerbound for the minimax risk, based on an $f$-divergence and is proven in \cite[II.2]{guntuboyinaLowerBoundsMinimax2011}.
\begin{proposition}
